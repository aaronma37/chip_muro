...
tf2_msgs::TFMessage person_rotation;
...
void person_cb(const geometry_msgs::PoseStamped::ConstPtr& cenPose){
	formation_goal.x=cenPose->pose.position.x;
	formation_goal.y=-cenPose->pose.position.y;
	rotation=tf::getYaw(cenPose->pose.orientation);
}
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  ros::Publisher bellini_goal_publisher = n.advertise<geometry_msgs::PoseStamped>("/ardrone1/goal_pose", 1000);
  ros::Publisher michelangelo_goal_publisher = n.advertise<geometry_msgs::PoseStamped>("/ardrone2/goal_pose", 1000);
  //ros::Publisher bellini_goal_publisher = n.advertise<geometry_msgs::PoseStamped>("/ardrone3/goal_pose", 1000);
  //ros::Publisher leonardo_goal_publisher = n.advertise<geometry_msgs::PoseStamped>("/ardrone4/goal_pose", 1000);

  ros::Publisher goal_publisher = n.advertise<tf2_msgs::TFMessage>("/formationSpecifications_modified",1000);

  ros::Subscriber goal_subscriber = n.subscribe<tf2_msgs::TFMessage>("/formationSpecifications",1, read_from_android);

  ros::Subscriber bellini_goal_publisher_goal_subscriber = n.subscribe<geometry_msgs::PoseStamped>("/ardrone1/relativeGoal",1, bellini_cb);
  ros::Subscriber michelangelo_goal_subscriber = n.subscribe<geometry_msgs::PoseStamped>("/ardrone2/relativeGoal",1, michelangelo_cb);
  //ros::Subscriber bellini_goal_subscriber = n.subscribe<geometry_msgs::PoseStamped>("/ardrone3/relativeGoal",1, bellini_cb);
  //ros::Subscriber leonardo_goal_subscriber = n.subscribe<geometry_msgs::PoseStamped>("/ardrone4/relativeGoal",1, leonardo_cb);

  ros::Subscriber person_subscriber = n.subscribe<geometry_msgs::PoseStamped>("/ardrone3/afterKalman",1, person_cb);
...
//	formation_goal.z=0;
	...
	if (rotationFlag){
		person_rotation.transforms[0].transform.translation.z=rotation;
		goal_publisher.publish(person_rotation);
	}
