...
geometry_msgs::PoseStamped quad_poses;
...
void quad_cb(const geometry_msgs::PoseStamped::ConstPtr& posePtr)
{
    ...
    if (msg.header.frame_id=="ardrone1"){
	k=21;
    }else if (msg.header.frame_id=="ardrone2"){
	k=22;
    }else if (msg.header.frame_id=="ardrone3"){
	k=23;
    }else if (msg.header.frame_id=="ardrone4"){
	k=24;
    }

	    std::cout<<"FOUND ARDRONE" << k;

    measurementPose[k].header.frame_id=msg.header.frame_id;
    ...
    measurementPose[k].pose.position.x = msg.pose.position.y;
    measurementPose[k].pose.position.y = msg.pose.position.x;
    measurementPose[k].pose.position.z = msg.pose.position.z;


    measurementPose[k].pose.orientation.x =msg.pose.orientation.z;
    measurementPose[k].pose.orientation.y =msg.pose.orientation.x;
    measurementPose[k].pose.orientation.z = -msg.pose.orientation.y;


    measurementPose[k].pose.orientation.w = msg.pose.orientation.w;
...
void ar1_cmd(const geometry_msgs::Twist::ConstPtr& ipt){
	twist[21].linear=ipt->linear;
	twist[21].angular=ipt->angular;
}
void ar2_cmd(const geometry_msgs::Twist::ConstPtr& ipt){
	twist[22].linear=ipt->linear;
	twist[22].angular=ipt->angular;
}

void ar3_cmd(const geometry_msgs::Twist::ConstPtr& ipt){
	twist[23].linear=ipt->linear;
	twist[23].angular=ipt->angular;
}

void ar4_cmd(const geometry_msgs::Twist::ConstPtr& ipt){
	twist[24].linear=ipt->linear;
	twist[24].angular=ipt->angular;
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber ipt_sub_,ap_sub_ ,measured_sub_,multi_quad_sub;
    ...
    poseEstimation[21].header.frame_id="ardrone1";
    poseEstimation[22].header.frame_id="ardrone2";
    poseEstimation[22].header.frame_id="ardrone3";
    poseEstimation[23].header.frame_id="Typhoon";
...
poseEstimation[43].header.frame_id="goya";
    ...
    multi_quad_sub= nh_.subscribe<geometry_msgs::PoseStamped>("/ORB_SLAM", 25, quad_cb);
    //ipt_sub_=nh_.subscribe<tf2_msgs::TFMessage>("/cmd_vel",25,iptCallback);

    ros::Subscriber ardrone1_cmd=nh_.subscribe<geometry_msgs::Twist>("/ardrone1/cmd_vel",25,ar1_cmd);
    ros::Subscriber ardrone2_cmd=nh_.subscribe<geometry_msgs::Twist>("/ardrone2/cmd_vel",25,ar2_cmd);
    ros::Subscriber ardrone3_cmd=nh_.subscribe<geometry_msgs::Twist>("/ardrone3/cmd_vel",25,ar3_cmd);
    ros::Subscriber ardrone4_cmd=nh_.subscribe<geometry_msgs::Twist>("/ardrone4/cmd_vel",25,ar4_cmd);
    ...
    ros::Publisher ardrone1_pub = gnh_.advertise<geometry_msgs::PoseStamped>("/ardrone1/afterKalman", 1000, true);
    ros::Publisher ardrone2_pub = gnh_.advertise<geometry_msgs::PoseStamped>("/ardrone2/afterKalman", 1000, true);
    ros::Publisher ardrone3_pub = gnh_.advertise<geometry_msgs::PoseStamped>("/ardrone3/afterKalman", 1000, true);
    ros::Publisher ardrone4_pub = gnh_.advertise<geometry_msgs::PoseStamped>("/ardrone4/afterKalman", 1000, true);
 
    ros::Publisher ardrone1_pub_vel = gnh_.advertise<geometry_msgs::Twist>("/ardrone1/velocityEstimation", 1000, true);
    ros::Publisher ardrone2_pub_vel = gnh_.advertise<geometry_msgs::Twist>("/ardrone2/velocityEstimation", 1000, true);
    ros::Publisher ardrone3_pub_vel = gnh_.advertise<geometry_msgs::Twist>("/ardrone3/velocityEstimation", 1000, true);
    ros::Publisher ardrone4_pub_vel = gnh_.advertise<geometry_msgs::Twist>("/ardrone4/velocityEstimation", 1000, true);
...
//REMOVE
	...
	reset_index(0);
        for (int i=0;i<num;i++){
			...
			if(i==21){
				quad_poses.pose=poseEstimation[i].pose;
				ardrone1_pub.publish(quad_poses);

				quad_vel.linear.x=twistEstimation.transforms[i].transform.translation.x;
				quad_vel.linear.y=twistEstimation.transforms[i].transform.translation.y;
				quad_vel.linear.z=twistEstimation.transforms[i].transform.translation.z;

				quad_vel.angular.x=twistEstimation.transforms[i].transform.rotation.x;
				quad_vel.angular.y=twistEstimation.transforms[i].transform.rotation.y;
				quad_vel.angular.z=twistEstimation.transforms[i].transform.rotation.z;

				ardrone1_pub_vel.publish(quad_vel);

				std::cout<<"\n VEL Z" << twist[i].linear.z;


			}
			if(i==22){
				quad_poses.pose=poseEstimation[i].pose;
				ardrone2_pub.publish(quad_poses);
				...
				ardrone2_pub_vel.publish(quad_vel);
			}
			if(i==23){
				quad_poses.pose=poseEstimation[i].pose;
				ardrone3_pub.publish(quad_poses);
				...
				ardrone3_pub_vel.publish(quad_vel);
			}
			if(i==24){
				quad_poses.pose=poseEstimation[i].pose;
				ardrone4_pub.publish(quad_poses);
				...
				ardrone4_pub_vel.publish(quad_vel);
			}
