...
const double DEFAULT_KP = 1; //.5
const double DEFAULT_KI = 0.01;
const double DEFAULT_KD = 0.0;//.1
const double DEFAULT_KPZ = .1;
...
void poseEstCallback(const geometry_msgs::PoseStamped::ConstPtr& posePtr)
{
    ...
    poseEstYaw = tf::getYaw(poseEstimation.pose.orientation);
}
...
void poseGoalCallback(const geometry_msgs::PoseStamped::ConstPtr& posePtr)
{
    ...
    poseGoal.pose.position.y=-poseGoal.pose.position.y;
    poseGoal.pose.position.z=.25;
    poseGoalYaw = 0;
}
...
void calculateError(void)
{
    ...
    poseError.pose.orientation = tf::createQuaternionMsgFromYaw(poseErrYaw); 
        std::cout<<"\nerr z" << poseError.pose.position.z << "\n";
...
void PID(void)
{
    ...
    if (velocity.linear.y > 1){
      ...
      velocity.linear.z=kpZ*poseError.pose.position.z;
...
int main(int argc, char **argv)
{
    ...
    poseEstSub = n.subscribe<geometry_msgs::PoseStamped>("afterKalman", 1, poseEstCallback);
